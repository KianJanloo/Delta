...
const SecondStep: FC<ISecondStep> = ({
  ...
  const priceWithDiscount = discountPrice(
    Number(book?.house?.price),
  ...
  const handleBooking = async () => {
    ...
    if (response) {
      ...
      if (bookData.houseId) {
        const house = await getHouseById(bookData.houseId);

        const data = {
          userId: Number(house.sellerId),
          dataNotification: { emailPassenger: bookData.sharedEmail },
          title: " رزرو جدید ",
          message: ` رزرو جدیدی برای ${house.title} ثبت شده است `,
          type: "new_booking",
        };

        await sendNotif(data);
      }
  ...
  return (
    <div className="w-full flex flex-col gap-8">
      ...
      <div className="w-full bg-secondary-light2 px-2 py-2 gap-8 rounded-2xl flex flex-col">
        ...
        <span className="mx-4 font-light leading-10">{t("sideCostDesc")}</span>
      </div>
      ...
      <div className="w-full bg-secondary-light2 px-2 py-2 gap-8 rounded-2xl flex flex-col">
        ...
        <div className="px-4 w-full flex gap-4 items-center pb-4 flex-wrap">
          <div className="flex gap-2 items-center">
            ...
            >
              <path
                d="M4.06215 1.53451C4.38431 0.663888 5.61569 0.663888 5.93785 1.53451L6.46097 2.94821C6.56226 3.22193 6.77807 3.43774 7.05179 3.53903L8.46549 4.06215C9.33611 4.38431 9.33611 5.61569 8.46549 5.93785L7.05179 6.46097C6.77807 6.56226 6.56226 6.77807 6.46097 7.05179L5.93785 8.46549C5.61569 9.33611 4.38431 9.33611 4.06215 8.46549L3.53903 7.05179C3.43774 6.77807 3.22193 6.56226 2.94821 6.46097L1.53451 5.93785C0.663888 5.61569 0.663888 4.38431 1.53451 4.06215L2.94821 3.53903C3.22193 3.43774 3.43774 3.22193 3.53903 2.94821L4.06215 1.53451Z"
                fill="white"
              />
            </svg>
            <span className="text-primary">
              {" "}
              <span className="text-foreground">{t("phoneNumber")}:</span>{" "}
              {book?.sharedMobile}{" "}
            </span>
          </div>
          <svg
      ...
      <div className="relative w-full max-lg:mt-[30px] flex justify-center items-center">
        ...
        <div className="w-full flex absolute justify-between flex-wrap gap-4 max-w-[1300px] px-4 py-2 text-lg md:text-2xl">
          <div className="flex gap-3 items-center">
            ...
            <span>
              ...
              <span className="text-primary">
                {" "}
                {SplitNumber(
                  book?.discount ? priceWithDiscount : book?.house?.price || 0
                )}{" "}
          ...
          <div className="flex gap-4">
            <CommonButton
              onclick={handleCurrentSteDecrease}
              title={t("prevStep")}
              icon={<ChevronRight size={16} />}
              classname="bg-transparent border-foreground flex flex-row-reverse border w-fit text-foreground"
            />
            <CommonButton
              onclick={handleBooking}
              title={loading ? "در حال بارگزاری" : t("onlinePayment")}
