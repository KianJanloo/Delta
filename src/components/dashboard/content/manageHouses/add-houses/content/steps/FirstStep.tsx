...
const FirstStep = ({
  ...
  const [categoryData, setCategoryData] = useState<{ name: string } | null>(
    house.categories ? { name: house.categories.name } : null
  );
    ...
    >
      <div className="flex max-lg:flex-col w-full justify-between gap-8">
        <div className="w-1/2 max-lg:w-full flex flex-col gap-2">
          <Label htmlFor="title" className="text-subText">
            {t("houseName")}
          </Label>
          <Input
        ...
        <div className="w-1/2 max-lg:w-full flex flex-col gap-2">
          <Label htmlFor="capacity" className="text-subText">
            {t("capacity")}
          </Label>
          <Input
      ...
      <div className="flex max-lg:flex-col w-full justify-between gap-8">
        ...
        <div className="w-1/2 max-lg:w-full flex flex-col gap-2">
          <Label htmlFor="price" className="text-subText">
            {t("price")}
          </Label>
          <Input
      ...
      <div className="flex max-lg:flex-col w-full justify-between gap-8 items-center">
        <div className="w-1/2 max-lg:w-full flex flex-col gap-2">
          <Label htmlFor="category" className="text-subText">
            {t("type")}
          </Label>
          <Input
            defaultValue={house.categories?.name}
            onChange={async (e) => {
        ...
        >
          <path
            d="M21 5H9V1.43484C9 1.26397 8.79958 1.17179 8.66984 1.28299L2 7L8.66984 12.717C8.79958 12.8282 9 12.736 9 12.5652V9H21"
            className="stroke-foreground"
            strokeWidth="2"
          />
        </svg>
        <div className="w-1/2 max-lg:w-full flex flex-col gap-2">
          <Label htmlFor="subtype" className="text-subText">
            {t("subtype")}
          </Label>
          <Input
            name="subtype"
      ...
      <div className="w-full max-lg:w-full flex flex-col gap-2">
        <Label htmlFor="caption" className="text-subText">
          {t("description")}
        </Label>
        <Textarea
          defaultValue={house.caption}
          {...register("caption")}
          name="caption"
          placeholder={t("descriptionPlaceholder")}
          id="caption"
          className="w-full h-[244] rounded-xl border border-subText text-subText"
        />
        {errors.caption && (
          <span className="text-xs text-danger">
            {" "}
